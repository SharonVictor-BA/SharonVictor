...
st.markdown("Predict CO2 emissions based on supply chain and transport activity using Random Forest.")

# Upload dataset
uploaded_file = st.file_uploader("Upload the CO2 Emission Dataset (.csv)", type=['csv'])
...
if uploaded_file:
    ...
    categorical_cols = ['Facility Type', 'Emission Source', 'Transport Mode', 'Material Type', 'Supply Chain Activity']
    ...
    selected_target = st.sidebar.selectbox("Select Target Variable to Predict", targets)

    # Dropdowns based on unique values in data
    date_input = st.sidebar.date_input("Select Prediction Date", value=datetime(2027, 1, 1))

    user_input = {
        'Facility Type': st.sidebar.selectbox("Facility Type", sorted(df['Facility Type'].dropna().unique())),
        'Emission Source': st.sidebar.selectbox("Emission Source", sorted(df['Emission Source'].dropna().unique())),
        'Transport Mode': st.sidebar.selectbox("Transport Mode", sorted(df['Transport Mode'].dropna().unique())),
        'Material Type': st.sidebar.selectbox("Material Type", sorted(df['Material Type'].dropna().unique())),
        'Supply Chain Activity': st.sidebar.selectbox("Supply Chain Activity", sorted(df['Supply Chain Activity'].dropna().unique())),
    ...
    st.success(f"{selected_target}: {prediction:.2f}")
